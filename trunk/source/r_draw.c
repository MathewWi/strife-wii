 ...
 *-----------------------------------------------------------------------------
 *
 * $Id: r_draw.c,v 1.13.2.3 2002/07/20 18:08:37 proff_fs Exp $
 *
...
static const char
rcsid[] = "$Id: r_draw.c,v 1.13.2.3 2002/07/20 18:08:37 proff_fs Exp $";
...
#define MAXTRANS 7
  ...
  xlatabtable = W_CacheLumpName("XLATAB");	//kaiser
  ...
  for (i=0; i<MAXPLAYERS; i++) {
    ...
    if (wantcolour != 0x80) // Not green, would like translation
  ...
  if (transtocolour[j] == 255) {
	...
	//updated 10-9-05	finally fixed the multible translation tables -kaiser
  // translate just the 16 green colors
  for (i=0; i<256; i++)
  {
    if (i >= 0x80 && i<= 0x8f)	//kaiser
      {
  // CPhipps - configurable player colours
        translationtables [i      ] = 0x40 + (i&0xf);
        translationtables [i+  256] = 0xb0 + (i&0xf);
        translationtables [i+2*256] = 0x30 + (i&0xf);
		translationtables [i+3*256] = 0x10 + (i&0xf);
		translationtables [i+4*256] = 0x50 + (i&0xf);
		translationtables [i+5*256] = 0x60 + (i&0xf);
		translationtables [i+6*256] = 0x90 + (i&0xf);
      }

	else if (i >= 0x50 && i<= 0x5f)	//kaiser
      {
        translationtables [i      ] = 0x50 + (i&0xf);
        translationtables [i+  256] = 0x50 + (i&0xf);
        translationtables [i+2*256] = 0x50 + (i&0xf);
		translationtables [i+3*256] = 0x50 + (i&0xf);
		translationtables [i+4*256] = 0x80 + (i&0xf);
		translationtables [i+5*256] = 0x10 + (i&0xf);
		translationtables [i+6*256] = 0x40 + (i&0xf);
      }
	else if (i >= 0xd0 && i<= 0xdf)	//kaiser
      {
        translationtables [i      ] = 0xd0 + (i&0xf);
        translationtables [i+  256] = 0xd0 + (i&0xf);
        translationtables [i+2*256] = 0xd0 + (i&0xf);
		translationtables [i+3*256] = 0xd0 + (i&0xf);
		translationtables [i+4*256] = 0xb0 + (i&0xf);
		translationtables [i+5*256] = 0x30 + (i&0xf);
		translationtables [i+6*256] = 0x10 + (i&0xf);
      }
	else if (i >= 0xc0 && i<= 0xcf)	//kaiser
      {
        translationtables [i      ] = 0xc0 + (i&0xf);
        translationtables [i+  256] = 0xc0 + (i&0xf);
        translationtables [i+2*256] = 0xc0 + (i&0xf);
		translationtables [i+3*256] = 0xc0 + (i&0xf);
		translationtables [i+4*256] = 0xa0 + (i&0xf);
		translationtables [i+5*256] = 0x20 + (i&0xf);
		translationtables [i+6*256] = 0x00 + (i&0xf);
      }
    else  // Keep all other colors as is.
      translationtables[i]=translationtables[i+256]=translationtables[i+2*256]=
	  translationtables[i+3*256]=translationtables[i+4*256]=translationtables[i+5*256]=
...
void R_FillBackScreen (void)
{
  ...
  V_DrawBackground(gamemode == commercial ? "F_KNTBRK" : "FLOOR7_2", 1);	//kaiser
