...
boolean PIT_CrossLine (line_t* ld)
  ...
  if (!(ld->flags & ML_TWOSIDED) ||
      (ld->flags & (ML_BLOCKING|ML_BLOCKMONSTERS|ML_BLOCKALL)))	//kaiser
...
boolean PIT_CheckLine (line_t* ld)
{
  ...
  if (!(tmthing->flags & (MF_MISSILE | MF_BOUNCES)) || (ld->flags & ML_BLOCKALL))//kaiser
    {
      if (ld->flags & ML_BLOCKING || ld->flags & ML_BLOCKALL)           // explicitly blocking everything
  return tmunstuck && !untouched(ld);  // killough 8/1/98: allow escape

      // killough 8/9/98: monster-blockers don't affect friends
      if (!(tmthing->flags & (MF_FRIEND|MF_GHOST) || tmthing->player)
    && ld->flags & ML_BLOCKMONSTERS)
  return false; // block monsters only

	  //this makes sure that the player is still jumping before the line
	  //deactivates itself - kaiser
	  passline1 = sides[ld->sidenum[0]].midtexture;
	  passline2 = sides[ld->sidenum[1]].midtexture;
	  if ((tmthing->player)
    && ld->flags & (ML_JUMPOVER|ML_BLOCKING)
	&& sides[ld->sidenum[0]].midtexture > 0 &&
			sides[ld->sidenum[1]].midtexture > 0)
	  {
		if(tmthing->player->mo->z <=
			textureheight[passline1]+tmthing->player->mo->floorz
			&& tmthing->player->mo->z <=
			textureheight[passline2]+tmthing->player->mo->floorz)
		return false;
	  }
...
static boolean PIT_CheckThing(mobj_t *thing) // killough 3/26/98: make static
{
  ...
  if (!(thing->flags & (MF_SOLID|MF_SPECIAL|MF_SHOOTABLE|MF_PEASANT)))
    return true;
  
  //kaiser
  if ((thing->flags & (MF_GHOST)) && (tmthing->flags & (MF_MISSILE))
	  && !(tmthing->flags & (MF_GHOST)))
	  return true;	//missiles goes right though ghosts except ghost missiles -kaiser

  if ((thing->player || thing->flags & MF_FRIEND) && tmthing->flags & (MF_GHOST)
	  && (!(tmthing->flags & (MF_SOLID))))
	  return true;

  if ((thing->flags & MF_BOSS) && (tmthing->flags & MF_BOSS)
	  && !(tmthing->flags & (MF_GHOST)))
	  return true;	//boss projectiles goes right though bosses -kaiser
  ...
  //Height Clipping code - kaiser
  BlockingMobj = thing;
  if(tmthing->passmobj == true)
  {
  ...
  cmd = &tmthing->player->cmd;
  if(tmthing->player)
  {
  if (thing->flags & MF_PEASANT &&
      tmthing->flags & MF_SOLID &&
      thing->health > 0 &&
  ...
  if (tmthing->flags & MF_SKULLFLY)
    {
      ...
      P_SetMobjState (tmthing, tmthing->info->spawnstate);	//kaiser

      return true;   // stop moving
    }

  if(tmthing->flags & MF_MISSILE &&	//9-4-05	//kaiser
		  (tmthing->type == MT_HEGRENADE || tmthing->type == MT_PGRENADE)
		  && !thing->player && thing->flags & MF_SOLID
		  && tmthing->z < thing->height + thing->floorz)
  {
	  tmthing->momx = 0;
	  tmthing->momy = 0;
  }
  ...
  return true;    // underneath
    ...
    (tmthing->target->type == MT_CRUSADER && thing->type == MT_CRUSADER)||
    (tmthing->target->type == MT_SENTINEL && thing->type == MT_SENTINEL)))
      {
  if (thing == tmthing->target)
    ...
    if (thing->type != MT_PLAYER) // Explode, but do no damage.
      ...
      if (!(tmthing->flags & MF_MISSILE)) {	//kaiser
  ...
  return !(thing->flags & MF_SOLID); // didn't do any damage
      ...
      damage = ((P_Random(pr_damage)%4)+1)*tmthing->info->damage;	//kaiser
...
boolean P_TryMove(mobj_t* thing,fixed_t x,fixed_t y,
  ...
  if ( !(thing->flags & MF_NOCLIP) )
    ...
    (/*!(thing->flags & MF_TELEPORT) &&*/
     tmfloorz - thing->z > 16*FRACUNIT))	//kaiser
  return tmunstuck
...
boolean PTR_SlideTraverse (intercept_t* in)
  ...
  if (li->flags & (ML_BLOCKALL))	//kaiser
	{
		goto isblocking;
	}
...
/*  if ((th->flags&MF_GHOST))
    return true;    // can't shoot spectures -kaiser*/
...
boolean PTR_ShootTraverse (intercept_t* in)
  ...
  if (in->isaline)
    ...
    if (li->special)
	  ...
	  if(!(li->flags & ML_BLOCKALL))
      if (li->flags & ML_TWOSIDED && !(li->special == 182)) //kaiser
  {  // crosses a two sided (really 2s) line
	...
	if(li->special == 182 || li->special == 180)	//checks to see if linedef is type 182. if so then spawn
							// a mobj rather than a bullet puff!  -kaiser
	{
		P_BreakingGlass(li);
	}
  ...
  if ((th->flags&MF_NOSECTOR))
	  return true;	//if its invisible, don't shoot it	-kaiser
  ...
  if(PuffType != MT_STRIFEPUFF3)
  PuffType = MT_SPARKPUFF;	//kaiser
  if (in->d.thing->flags & MF_NOBLOOD || PuffType == MT_STRIFEPUFF3)
    P_SpawnPuff (x,y,z);
  else
...
boolean PTR_UseThingTraverse (intercept_t* in)	//kaiser
{
	mobj_t *mobj;
	char *name;
	...
	if(mobj->disable_npc)
		return true;
	//	Check height.
		if (usething->z >= in->d.thing->z + in->d.thing->height ||
			usething->z + usething->height <= in->d.thing->z)
		{
			return true;
		}
		if(mobj->talktics)	//9-06-05	-kaiser
		{
			return false;
		}
		if(!mobj->player && P_CheckForClass(mobj,CLASS_SHOPGUY) ||
		P_CheckForClass(mobj,CLASS_ACOLYTE) || P_CheckForClass(mobj,CLASS_PEASANT)
		|| P_CheckForClass(mobj,CLASS_BEGGAR) || P_CheckForClass(mobj,CLASS_REBEL)
		|| P_CheckForClass(mobj,CLASS_TEMPLER) || mobj->type == MT_RLEADER
		|| mobj->type == MT_RLEADER2 || mobj->type == MT_ORACLE)
		/*(mobj->flags & (MF_SOLID|MF_SHOOTABLE|MF_COUNTKILL|MF_PEASANT
			|MF_NOALERT)))*/ //must have the following flags to pass -kaiser
	{ // Wrong special
		S_StartSound(mobj,sfx_radio);
		name = mobj->info->stringname;

		if(P_CheckForClass(mobj,CLASS_SHOPGUY))
			P_SetMobjState(mobj,S_SHOP_27);
		mobj->oldangle = mobj->angle;
		mobj->angle = R_PointToAngle2(mobj->x, mobj->y,
        usething->x, usething->y);
		usething->player->momx = 0;
		usething->player->momy = 0;
		usething->player->momz = 0;
		usething->player->centering = true;
		usething->angle = R_PointToAngle2(usething->x, usething->y,
        mobj->x, mobj->y);
		if (C_StartConversation(usething, mobj))
		{
			return false;
		}
...
//
void P_UseLines (player_t *player)	//kaiser
{
	int                     angle;
	fixed_t         x1, y1, x2, y2;
	...
	x2 = x1 + (USETHINGRANGE>>FRACBITS)*finecosine[angle];
	y2 = y1 + (USETHINGRANGE>>FRACBITS)*finesine[angle];
...
boolean PIT_RadiusAttack (mobj_t* thing)
   ...
   */
  if(thing->flags & (MF_NOSECTOR|MF_GHOST))	//kaiser
	  return false;
  ...
  if (bombspot->flags & MF_BOUNCES ?
      thing->type == MT_INQUISITOR && bombsource->type == MT_INQUISITOR :
      thing->type == MT_INQUISITOR)	//kaiser
    return true;
...
boolean PIT_RadiusAttack2 (mobj_t* thing)
  {
  fixed_t dx;
  fixed_t dy;
  fixed_t dist;
...
void P_RadiusAttack2(mobj_t* spot,mobj_t* source,int damage)
  {
  int x;
  int y;
  ...
  for (y=yl ; y<=yh ; y++)
    for (x=xl ; x<=xh ; x++)
      P_BlockThingsIterator (x, y, PIT_RadiusAttack2 );
  }
...
static void P_CrunchMobj(mobj_t* thing)
{
	mobjtype_t bodypart;
	mobj_t* mo;
	int i;
  ...
  mo = P_TossUpMobj(thing, bodypart);
  for(i = 0; i < 16; i++)
  {
