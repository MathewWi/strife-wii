...
const char skullName[8][/*8*/9] = {"M_CURS1","M_CURS2","M_CURS3","M_CURS4","M_CURS5"
									,"M_CURS6","M_CURS7","M_CURS8"};	//kaiser
...
void M_NameCharacter(int choice);	//kaiser
...
void M_ToggleVoices(int choice);	//kaiser
...
void M_WriteDialog(int x, int y, int w, const char *string);
...
void M_ByeCancel(int choice);	//kaiser
void M_Choice1(int choice);	//kaiser
void M_Choice2(int choice);	//kaiser
void M_Choice3(int choice);
void M_Choice4(int choice);
void M_Choice5(int choice);
void M_Cancel(int choice);
void M_UnLoadName(int input, int paragraph);
void M_UnLoadSpeech(int input, int paragraph);
void M_UnLoadVoice(int input, int paragraph);
void M_UnLoadPanel(int input, int paragraph);
void M_UnLoadChoices(int input, int paragraph);
void M_UnLoadChoices2(int input, int paragraph);
void M_UnLoadChoices3(int input, int paragraph);
void M_UnLoadChoices4(int input, int paragraph);
void M_UnLoadChoices5(int input, int paragraph);
...
void M_DrawMainMenu(void)
{
  ...
  V_DrawNamePatch(88, 2, 0, "M_STRIFE", CR_DEFAULT, VPT_STRETCH);	//kaiser
}
...
{
  // CPhipps - patch drawing updated
  V_DrawNamePatch(96, 14, 0, "M_NGAME", CR_DEFAULT, VPT_STRETCH);	//kaiser
...
static void M_VerifyNightmare(int ch)
{
  ...
  if(strifedemo)
  {
	  G_DeferedInitNew(nightmare,33);	//kaiser
  }
  else
  {
  G_DeferedInitNew(nightmare,2);	//kaiser
  }
  finaletype = noend;
  M_ClearMenus ();
}
...
void M_ChooseSkill(int choice)
{
  ...
  if(strifedemo)
  {
	  G_DeferedInitNew(choice,33);	//kaiser
  }
  else
  {
  G_DeferedInitNew(choice,2);	//kaiser
  }
  finaletype = noend;
  M_ClearMenus ();
}
...
//    char name[PATH_MAX+1];    // killough 3/22/98
	...
	sprintf (namebuf, "%s/savegame/STRFSAV%i/NAME", basesavegame, i);
...
void M_DrawSave(void)
{
  ...
  if(NeedsNameInput)
	  M_WriteText(72,LOADGRAPHIC_Y,"Name Your Character");
  else
...
void M_SaveGame (int choice)
{
  ...
  M_ClearMenus();		//automatically saves slot, since the slot# was already set.-kaiser
  M_DoSave(saveSlot);
}

void M_NameCharacter (int choice)
{
  NeedsNameInput = true;
...
char vocNames[2][9]  = {"M_MSGON","M_MSGOFF"};
...
void M_DrawOptions(void)
{
  ...
  V_DrawNamePatch(108, 15, 0, "M_OPTION", CR_DEFAULT, VPT_STRETCH);	//kaiser
...
///////////////////////////////////////////////////////////////////
//			Scripted Conversation Menu		-kaiser
///////////////////////////////////////////////////////////////////
//1-11-06	simplified
typedef struct {
	char speechTxt[256];
	...
	char nameTxt[256];
	char panelTxt[256];
	char choiceTxt[5][256];
	boolean gotvoice;
	boolean gotpanel;
	boolean gotChoices[5];
	int check;
	int startNewSpeech[5];
	boolean onetimer;
	int cancelinteger;
} menuSpeech_t;
...
menu_t ScriptTalkDef =
{
  script_end,       // number of menu items
  NULL,           // previous menu screen
  ScriptTalkMenu,       // table that defines menu items
  M_DrawScriptTalk, // drawing routine
  56,132,          // initial cursor position
  0               // last menu item the user was on
};
...
menuitem_t ScriptTalkMenu2[]=
{
  {1,"", M_Choice1, '1'},
  {1,"", M_ByeCancel, '2'},
};

menu_t ScriptTalkDef2 =
{
  script_end_2,       // number of menu items
  NULL,           // previous menu screen
  ScriptTalkMenu2,       // table that defines menu items
  M_DrawScriptTalk, // drawing routine
  56,148,          // initial cursor position
  0               // last menu item the user was on
};
...
menuitem_t ScriptTalkMenu3[]=
{
  {1,"", M_ByeCancel, '1'},
};

menu_t ScriptTalkDef3 =
{
  script_end_3,       // number of menu items
  NULL,           // previous menu screen
  ScriptTalkMenu3,       // table that defines menu items
  M_DrawScriptTalk, // drawing routine
  56,164,          // initial cursor position
  0               // last menu item the user was on
};
...
menu_t ScriptTalkDef4 =
{
  script4_end,       // number of menu items
  NULL,           // previous menu screen
  ScriptTalkMenu4,       // table that defines menu items
  M_DrawScriptTalk, // drawing routine
  56,116,          // initial cursor position
  0               // last menu item the user was on
};
...
menu_t ScriptTalkDef5 =
{
  script5_end,       // number of menu items
  NULL,           // previous menu screen
  ScriptTalkMenu5,       // table that defines menu items
  M_DrawScriptTalk, // drawing routine
  56,100,          // initial cursor position
  0               // last menu item the user was on
};
...
menu_t ScriptTalkDef6 =
{
  script6_end,       // number of menu items
  NULL,           // previous menu screen
  ScriptTalkMenu6,       // table that defines menu items
  M_DrawScriptTalk, // drawing routine
  56,84,          // initial cursor position
  0               // last menu item the user was on
};

void M_ByeCancel(int choice)
{
	currentMenu->lastOn = itemOn;
    M_ClearMenus ();
    S_StartSound(NULL,sfx_mtalht);	//kaiser
	C_StopSpeech();
}


void M_DrawScriptTalk(void)
{
	int borderline = 112;
	if(svMenu.gotpanel)
	V_DrawNamePatch(0, 0, 0, svMenu.panelTxt, CR_DEFAULT, VPT_STRETCH);
	if(svMenu.gotChoices[4])
	{
		borderline -= 32;
		M_WriteText(50,96,svMenu.choiceTxt[0]);
		M_WriteText(50,112,svMenu.choiceTxt[1]);
		M_WriteText(50,128,svMenu.choiceTxt[2]);
		M_WriteText(50,144,svMenu.choiceTxt[3]);
		M_WriteText(50,160,svMenu.choiceTxt[4]);
	}
	else if(svMenu.gotChoices[3])
	{
		borderline -= 16;
		M_WriteText(50,112,svMenu.choiceTxt[0]);
		M_WriteText(50,128,svMenu.choiceTxt[1]);
		M_WriteText(50,144,svMenu.choiceTxt[2]);
		M_WriteText(50,160,svMenu.choiceTxt[3]);
	}
	else if(svMenu.gotChoices[2])
	{
		M_WriteText(50,128,svMenu.choiceTxt[0]);
		M_WriteText(50,144,svMenu.choiceTxt[1]);
		M_WriteText(50,160,svMenu.choiceTxt[2]);
	}
	else if((svMenu.gotChoices[0]) && svMenu.gotChoices[1])
	{
		borderline += 16;
		M_WriteText(50,144,svMenu.choiceTxt[0]);
		M_WriteText(50,160,svMenu.choiceTxt[1]);
	}
	else if(svMenu.gotChoices[0])
	{
		borderline += 32;
		M_WriteText(50,160,svMenu.choiceTxt[0]);
	}
	else borderline += 48;
	M_WriteText(30,borderline,"______________________________");
	M_WriteDialog(16,32,280,svMenu.dialogTxt);
	M_WriteText(16,16,svMenu.nameTxt);
	M_WriteText(50,176,canceltext[svMenu.cancelinteger]);
}

void M_Choice1(int choice)
{
  currentMenu->lastOn = itemOn;
  M_ClearMenus ();
  C_ConChoiceImpulse(choice + 1);
	
}

void M_Choice2(int choice)
{
  currentMenu->lastOn = itemOn;
  M_ClearMenus ();
  C_ConChoiceImpulse(choice + 1);
...
void M_StartDialog(int input, int paragraph)	//dump everything from the script lumps
{
  svMenu.check = input;
  svMenu.cancelinteger = S_Random()%3;
  svMenu.gotvoice = false;
  svMenu.gotpanel = false;
  svMenu.gotChoices[0] = false;
  svMenu.gotChoices[1] = false;
  svMenu.gotChoices[2] = false;
  svMenu.gotChoices[3] = false;
  svMenu.gotChoices[4] = false;
  M_UnLoadSpeech(input, paragraph);
  M_UnLoadName(input, paragraph);
  M_UnLoadVoice(input, paragraph);
  M_UnLoadPanel(input, paragraph);
  M_UnLoadChoices(input, paragraph);
  M_UnLoadChoices2(input, paragraph);
  M_UnLoadChoices3(input, paragraph);
  M_UnLoadChoices4(input, paragraph);
  M_UnLoadChoices5(input, paragraph);
  
  NewDef.lastOn = defaultskill - 1;
  menuactive = 1;
  if(svMenu.gotChoices[0] && svMenu.gotChoices[1] && svMenu.gotChoices[2] && svMenu.gotChoices[3] && svMenu.gotChoices[4])
  {
	currentMenu = &ScriptTalkDef6;
  }
  else if(svMenu.gotChoices[0] && svMenu.gotChoices[1] && svMenu.gotChoices[2] && svMenu.gotChoices[3])
  {
	currentMenu = &ScriptTalkDef5;
  }
  else if(svMenu.gotChoices[0] && svMenu.gotChoices[1] && svMenu.gotChoices[2])
  {
	currentMenu = &ScriptTalkDef4;
  }
  else if(svMenu.gotChoices[0] && svMenu.gotChoices[1])
  {
	currentMenu = &ScriptTalkDef;
  }
  else if(svMenu.gotChoices[0])
  {
	  currentMenu = &ScriptTalkDef2;
  }
  else
  {
	  currentMenu = &ScriptTalkDef3;
  }
  itemOn = currentMenu->lastOn;
  print_warning_about_changes = false;
  if(svMenu.gotvoice)
//	  C_StartTalking(speech);
  S_StartVoice(NULL,svMenu.speechTxt);
}

void M_UnLoadName(int num, int paragraph)
{
  int scriptname;
  ...
  if(scriptname)
  {
	  ...
	  sprintf(svMenu.nameTxt,"%s",Speech->Name);
	  if (!Speech->Name[0])
		{
		  sprintf(svMenu.nameTxt,"PERSON");	//no names defaults to this	-kaiser
		}
  }
  else
  {
	  scriptname = C_GetSpeechIndex(GenericSpeeches, NumGenericSpeeches,
		num, paragraph);
  if(scriptname)
  {
	  Speech = &GenericSpeeches[scriptname - 1];
	  sprintf(svMenu.nameTxt,"%s",Speech->Name);
	  if(!Speech->Name[0])
	  {
		  sprintf(svMenu.nameTxt,"%s",defaultname);	//from p_map.h		-kaiser
	  }
  }
  }
}

void M_UnLoadSpeech(int num, int paragraph)
{
  int SpeechNum, spk;
  RogueConSpeech_t *Speech;

  SpeechNum = C_GetSpeechIndex(LevelSpeeches, NumLevelSpeeches,
		num, paragraph);

	if (SpeechNum)
	{
		if (SpeechNum > NumLevelSpeeches)
			return;
		Speech = &LevelSpeeches[SpeechNum - 1];
		sprintf(svMenu.dialogTxt,"%s",Speech->Text);
	}
	else	//sets up the global names by default, if none defined in a script	-kaiser
	{
	  SpeechNum = C_GetSpeechIndex(GenericSpeeches, NumGenericSpeeches,
		num, paragraph);
		if(SpeechNum)
		{
			Speech = &GenericSpeeches[SpeechNum - 1];
			sprintf(svMenu.dialogTxt,"%s",Speech->Text);
			for(spk = 0; spk < 55; spk += 11)
			{
				if(!strcmp(svMenu.dialogTxt, dialogText[spk]))
				{
					sprintf(svMenu.dialogTxt,"%s",dialogText[spk + 1 + S_Random()%9]);
				}
			}
		}
	}
...
void M_UnLoadVoice(int num, int paragraph)
{
  int scriptname;
  RogueConSpeech_t *Speech;
  ...
  {
	  Speech = &LevelSpeeches[scriptname - 1];
	  sprintf(svMenu.speechTxt,"%s",Speech->Voice);
	  if(W_CheckNumForName(svMenu.speechTxt) == -1)
		return;
	  svMenu.gotvoice = true;
  }
}


void M_UnLoadPanel(int num, int paragraph)
{
  int scriptname;
  RogueConSpeech_t *Speech;
  ...
  {
	  Speech = &LevelSpeeches[scriptname - 1];
	  sprintf(svMenu.panelTxt,"%s",Speech->BackPic);
	  if(W_CheckNumForName(svMenu.panelTxt) == -1)
		return;
	  svMenu.gotpanel = true;
  }
}

void M_UnLoadChoices(int num, int paragraph)
{
  int scriptname;
  RogueConSpeech_t *Speech;
  ...
  {
	  Speech = &LevelSpeeches[scriptname - 1];
	  if(Speech->Choices[0].NeedAmount1)
	  {
		sprintf(svMenu.choiceTxt[0],"1) %s for %d",Speech->Choices[0].Text,
			Speech->Choices[0].NeedAmount1);
	  }
	  else sprintf(svMenu.choiceTxt[0],"1) %s",Speech->Choices[0].Text);
	  if (!Speech->Choices[0].Text[0])
		{
		  return;
		}
	  ...
	  if (Choice->Next < 0)
	  {
		  svMenu.startNewSpeech[0] = Choice->Next;
	  }
	  else
	  {
		  svMenu.startNewSpeech[0] = Choice->Next;
		  svMenu.onetimer = true;
	  }
	  svMenu.gotChoices[0] = true;
  }
}

void M_UnLoadChoices2(int num, int paragraph)
{
  int scriptname;
  RogueConSpeech_t *Speech;
  RogueConChoice_t *Choice;
  ...
  {
	  Speech = &LevelSpeeches[scriptname - 1];
	  if(Speech->Choices[1].NeedAmount1)
	  {
		sprintf(svMenu.choiceTxt[1],"2) %s for %d",Speech->Choices[1].Text,
			Speech->Choices[1].NeedAmount1);
	  }
	  else sprintf(svMenu.choiceTxt[1],"2) %s",Speech->Choices[1].Text);
	  if (!Speech->Choices[1].Text[0])
		{
		  return;
		}
	  Choice = &Speech->Choices[1];
	  if (Choice->Next < 0)
	  {
		  svMenu.startNewSpeech[1] = Choice->Next;
	  }
	  else
	  {
		  svMenu.startNewSpeech[1] = Choice->Next;
	  ...
	  svMenu.gotChoices[1] = true;
  }
}

void M_UnLoadChoices3(int num, int paragraph)
{
  int scriptname;
  RogueConSpeech_t *Speech;
  RogueConChoice_t *Choice;
  ...
  {
	  Speech = &LevelSpeeches[scriptname - 1];
	  if(Speech->Choices[2].NeedAmount1)
	  {
		sprintf(svMenu.choiceTxt[2],"3) %s for %d",Speech->Choices[2].Text,
			Speech->Choices[2].NeedAmount1);
	  }
	  else sprintf(svMenu.choiceTxt[2],"3) %s",Speech->Choices[2].Text);
	  if (!Speech->Choices[2].Text[0])
		{
		  return;
		}
	  Choice = &Speech->Choices[2];
	  if (Choice->Next < 0)
	  {
		  svMenu.startNewSpeech[2] = Choice->Next;
	  }
	  else
	  {
		  svMenu.startNewSpeech[2] = Choice->Next;
	  ...
	  svMenu.gotChoices[2] = true;
  }
}

void M_UnLoadChoices4(int num, int paragraph)
{
  int scriptname;
  RogueConSpeech_t *Speech;
  RogueConChoice_t *Choice;
  ...
  {
	  Speech = &LevelSpeeches[scriptname - 1];
	  if(Speech->Choices[3].NeedAmount1)
	  {
		sprintf(svMenu.choiceTxt[3],"4) %s for %d",Speech->Choices[3].Text,
			Speech->Choices[3].NeedAmount1);
	  }
	  else sprintf(svMenu.choiceTxt[3],"4) %s",Speech->Choices[3].Text);
	  if (!Speech->Choices[3].Text[0])
		{
		  return;
		}
	  Choice = &Speech->Choices[3];
	  if (Choice->Next < 0)
	  {
		  svMenu.startNewSpeech[3] = Choice->Next;
	  }
	  else
	  {
		  svMenu.startNewSpeech[3] = Choice->Next;
	  ...
	  svMenu.gotChoices[3] = true;
  }
}

void M_UnLoadChoices5(int num, int paragraph)
{
  int scriptname;
  RogueConSpeech_t *Speech;
  RogueConChoice_t *Choice;
  ...
  {
	  Speech = &LevelSpeeches[scriptname - 1];
	  if(Speech->Choices[4].NeedAmount1)
	  {
		sprintf(svMenu.choiceTxt[4],"5) %s for %d",Speech->Choices[4].Text,
			Speech->Choices[4].NeedAmount1);
	  }
	  else sprintf(svMenu.choiceTxt[4],"5) %s",Speech->Choices[4].Text);
	  if (!Speech->Choices[4].Text[0])
		{
		  return;
		}
	  Choice = &Speech->Choices[4];
	  if (Choice->Next < 0)
	  {
		  svMenu.startNewSpeech[4] = Choice->Next;
	  }
	  else
	  {
		  svMenu.startNewSpeech[4] = Choice->Next;
...
void M_QuitResponse(int ch)
{
  ...
  int i;
  static char *quitsounds[]=	//very messy...-kaiser
{
	"QFMRM1",
	"QFMRM2",
	"QFMRM3",
  ...
  finaletype = quitgame;
  if (!nosfxparm && snd_card) // avoid delay if no sound card
    {
      S_StartVoice(NULL,quitsounds[(gametic>>2)&7]);
    }
  D_StartEndPic();	//kaiser
}
...
menuitem_t SoundMenu[]=
{
  ...
  {-1,"",0},
  {1,"M_VOIVOL",  M_ToggleVoices,'v'},	//temp.	-kaiser
  {-1,"",0}
};
...
void M_DrawSound(void)
{
  ...
  V_DrawNamePatch(SoundDef.x + 170, SoundDef.y+LINEHEIGHT*(sfx_empty3), 0,
      vocNames[playvoices], CR_DEFAULT, VPT_STRETCH);
}
...
{
	choice = 0;
	playvoices = 1 - playvoices;

	if(playvoices)
	voice_shutthefuckup = true;
	else
		voice_shutthefuckup = false;
}
...
#define CR_TITLE  CR_RED	//kaiser
#define CR_SET    CR_GREEN
#define CR_ITEM   CR_GOLD
#define CR_HILITE CR_AQUA
...
char ResetButtonName[8][16] = {"M_CURS1","M_CURS2","M_CURS3","M_CURS4","M_CURS5"
								,"M_CURS6","M_CURS7","M_CURS8"};
...
void M_DrawDefVerify()
{
  ...
  if (whichSkull) { // blink the text
    ...
    M_DrawMenuString(VERIFYBOXXORG+8,VERIFYBOXYORG+8,CR_GOLD);	//kaiser
  }
}
...
void M_DrawInstructions()
{
      ...
      if (!(setup_gather | print_warning_about_changes | demoplayback))
  {
    ...
    M_DrawMenuString(4, 184-allow, CR_GOLD);		//kaiser
    strcpy(menu_buffer,
     "default setting, which is the one being edited here.");
    M_DrawMenuString(4, 192-allow, CR_GOLD);
    if (allow)
      {
        strcpy(menu_buffer,
         "However, changes made here will take effect now.");
        M_DrawMenuString(4, 192, CR_GOLD);
      }
  }
...
setup_menu_t keys_settings1[] =  // Key Binding screen strings	//kaiser
{
  ...
  {"JUMP"		 ,S_KEY		  ,m_scrn,KB_X,KB_Y+6*8,{&key_jump},&mousebjump},
  {"STRAFE LEFT" ,S_KEY       ,m_scrn,KB_X,KB_Y+7*8,{&key_strafeleft}},
  {"STRAFE RIGHT",S_KEY       ,m_scrn,KB_X,KB_Y+8*8,{&key_straferight}},
  {"STRAFE"      ,S_KEY       ,m_scrn,KB_X,KB_Y+9*8,{&key_strafe},&joybstrafe},
  {"AUTORUN"     ,S_KEY       ,m_scrn,KB_X,KB_Y+10*8,{&key_autorun}},
  ...
  {"180 TURN"    ,S_KEY       ,m_scrn,KB_X,KB_Y+20*8,{&key_reverse}},
...
//  {"HUD"         ,S_KEY       ,m_scrn,KB_X,KB_Y+ 5*8,{&key_hud}},
  ...
  {"SAVE"        ,S_KEY       ,m_scrn,KB_X,KB_Y+11*8,{&key_savegame}},
  {"LOAD"        ,S_KEY       ,m_scrn,KB_X,KB_Y+12*8,{&key_loadgame}},
  {"QUICKSAVE"   ,S_KEY       ,m_scrn,KB_X,KB_Y+13*8,{&key_quicksave}},
  {"QUICKLOAD"   ,S_KEY       ,m_scrn,KB_X,KB_Y+14*8,{&key_quickload}},
  {"END GAME"    ,S_KEY       ,m_scrn,KB_X,KB_Y+15*8,{&key_endgame}},
  {"QUIT"        ,S_KEY       ,m_scrn,KB_X,KB_Y+16*8,{&key_quit}},
...
setup_menu_t keys_settings3[] =  // Key Binding screen strings
{
  ...
  {"CROSSBOW/POISON"  ,S_KEY       ,m_scrn,KB_X,KB_Y+ 2*8,{&key_weapon2}},
  {"ASSAULT GUN" ,S_KEY       ,m_scrn,KB_X,KB_Y+ 3*8,{&key_weapon3}},
  {"MINI ROCKET",S_KEY       ,m_scrn,KB_X,KB_Y+ 4*8,{&key_weapon4}},
  {"GRENADE/FIRE"  ,S_KEY       ,m_scrn,KB_X,KB_Y+ 5*8,{&key_weapon5}},
  {"FLAMETHROWER"  ,S_KEY       ,m_scrn,KB_X,KB_Y+ 6*8,{&key_weapon6}},
  {"MAULER",     S_KEY       ,m_scrn,KB_X,KB_Y+ 7*8,{&key_weapon7}},
  {"SIGIL",S_KEY       ,m_scrn,KB_X,KB_Y+ 8*8,{&key_weapon8}},
  {"OTHER"     ,S_KEY       ,m_scrn,KB_X,KB_Y+ 9*8,{&key_weapon9}},
  {"BEST"    ,S_KEY       ,m_scrn,KB_X,KB_Y+10*8,{&key_weapontoggle}},
  {"FIRE"    ,S_KEY       ,m_scrn,KB_X,KB_Y+11*8,{&key_fire},&mousebfire,&joybfire},
  {"INVENTORY" ,S_SKIP|S_TITLE,m_null,KB_X,KB_Y+12*8},
  {"SHOW STATUS SCREEN" ,S_KEY ,m_scrn,KB_X,KB_Y+13*8,{&key_showstat}},
  {"INVENTORY LEFT"	,S_KEY, m_scrn,KB_X,KB_Y+14*8,{&key_invleft}},
  {"INVENTORY RIGHT"	,S_KEY, m_scrn,KB_X,KB_Y+15*8,{&key_invright}},
  {"DROP INVENTORY"	,S_KEY, m_scrn,KB_X,KB_Y+16*8,{&key_dropinv}},
  {"USE INVENTORY"	,S_KEY, m_scrn,KB_X,KB_Y+17*8,{&key_useinv}},
  {"SHOW OBJECTIVES"	,S_KEY, m_scrn,KB_X,KB_Y+18*8,{&key_log}},
  {"SHOW KEYLIST"	,S_KEY, m_scrn, KB_X, KB_Y+19*8, {&key_showkeylist}},
...
setup_menu_t keys_settings5[] =  // Looking Up/Down/Center
{
  {"LOOKING" ,S_SKIP|S_TITLE,m_null,KB_X,KB_Y},
  {"LOOK UP"    ,S_KEY       ,m_scrn,KB_X,KB_Y+ 1*8,{&key_lookup}},
  {"LOOK DOWN"  ,S_KEY       ,m_scrn,KB_X,KB_Y+ 2*8,{&key_lookdown}},
  {"CENTER" ,S_KEY       ,m_scrn,KB_X,KB_Y+ 3*8,{&key_lookcenter}},
...

{
  ...
  M_DrawBackground("F_PAVE02", 0); // Draw background //KAISER
  // proff/nicolas 09/20/98 -- changed for hi-res
...
setup_menu_t weap_settings1[] =  // Weapons Settings screen
{
  ...
  {"Toggle Secondary Weapons", S_YESNO, m_null, WP_X,
...
#ifdef DOGS
  // killough 7/19/98
  {"Number Of Helper Allies",S_NUM|S_LEVWARN,m_null,E_X,E_Y+ enem_helpers*8, {"player_helpers"}},

  // killough 8/8/98
  {"Distance Friends Stay Away",S_NUM,m_null,E_X,E_Y+ enem_distfriend*8, {"friend_distance"}},

  {"Allow Allies to jump down",S_YESNO,m_null,E_X,E_Y+ enem_dog_jumping*8, {"dog_jumping"}},
#endif
...
//  {"HUD"         ,S_SKIP|S_KEY,m_null,KT_X1,KT_Y1+ 7*8,{&key_hud}},
  ...
  {"SCREENSHOT"  ,S_SKIP|S_KEY,m_null,KT_X1,KT_Y1+12*8,{&key_screenshot}},
   {"SHOW STATUS SCREEN" ,S_SKIP|S_KEY,m_null,KT_X1,KT_Y1+13*8,{&key_showstat}},
   {"INVENTORY LEFT" ,S_SKIP|S_KEY,m_null,KT_X1,KT_Y1+12*8,{&key_invleft}},
  {"INVENTORY RIGHT" ,S_SKIP|S_KEY,m_null,KT_X1,KT_Y1+13*8,{&key_invright}},
  {"DROP INVENTORY"	,S_SKIP|S_KEY, m_null,KT_X1,KT_Y1+14*8,{&key_dropinv}},
  ...
  {"USE INVENTORY"	,S_SKIP|S_KEY,m_null,KT_X3,KT_Y1+11*8,{&key_useinv}},
  {"FIRE"        ,S_SKIP|S_KEY,m_null,KT_X3,KT_Y1+12*8,{&key_fire},&mousebfire,&joybfire},
  ...
  {"JUMP"		 ,S_SKIP|S_KEY,m_null,KT_X3,KT_Y3+ 6*8,{&key_jump},&mousebjump},
  {"STRAFE LEFT" ,S_SKIP|S_KEY,m_null,KT_X3,KT_Y3+ 7*8,{&key_strafeleft}},
  {"STRAFE RIGHT",S_SKIP|S_KEY,m_null,KT_X3,KT_Y3+ 8*8,{&key_straferight}},
  {"STRAFE"      ,S_SKIP|S_KEY,m_null,KT_X3,KT_Y3+ 9*8,{&key_strafe},&joybstrafe},
  {"AUTORUN"     ,S_SKIP|S_KEY,m_null,KT_X3,KT_Y3+ 10*8,{&key_autorun}},
  ...
  {"180 TURN"    ,S_SKIP|S_KEY,m_null,KT_X2,KT_Y1+ 7*8,{&key_reverse}},
  // Final entry
...
setup_menu_t cred_settings[]={
  ...
  {"Programmer",S_SKIP|S_CREDIT|S_LEFTJUST,m_null, CR_X, CR_Y + CR_S*prog + CR_SH*cr_prog},
  {"Samuel Villarreal",S_SKIP|S_CREDIT|S_LEFTJUST,m_null, CR_X2, CR_Y + CR_S*(prog+1) + CR_SH*cr_prog}, //kaiser ^_^
  ...
  {"The Prboom Crew",S_SKIP|S_CREDIT|S_LEFTJUST,m_null, CR_X2, CR_Y + CR_S*(adcr+2)+CR_SH*cr_adcr},
  {"TeamTNT for BOOM",S_SKIP|S_CREDIT|S_LEFTJUST,m_null, CR_X2, CR_Y + CR_S*(adcr+3)+CR_SH*cr_adcr},
  {"Lee Killough for MBF",S_SKIP|S_CREDIT|S_LEFTJUST,m_null, CR_X2, CR_Y + CR_S*(adcr+4)+CR_SH*cr_adcr},
  {"Janis Legzdinsh",S_SKIP|S_CREDIT|S_LEFTJUST,m_null, CR_X2, CR_Y + CR_S*(adcr+5)+CR_SH*cr_adcr},
  {"Randy Heit",S_SKIP|S_CREDIT|S_LEFTJUST,m_null, CR_X2, CR_Y + CR_S*(adcr+6)+CR_SH*cr_adcr},
  {"Christoph Oelckers",S_SKIP|S_CREDIT|S_LEFTJUST,m_null, CR_X2, CR_Y + CR_S*(adcr+7)+CR_SH*cr_adcr},
  {"Rouge and Velocity for Strife",S_SKIP|S_CREDIT|S_LEFTJUST,m_null, CR_X2, CR_Y + CR_S*(adcr+8)+CR_SH*cr_adcr},
...
{
  inhelpscreens = true;
  M_DrawBackground(gamemode==shareware ? "CEIL5_1" : "F_BRNFLG", 0);	//fixed - kaiser
  V_DrawNamePatch(115,9,0, "SVSTRIFE",CR_GOLD, VPT_TRANS | VPT_STRETCH);	//kaiser
...
boolean M_Responder (event_t* ev) {
  ...
  {
    saveStringEnter = 0;
    if (savegamestrings[saveSlot][0] && NeedsNameInput == false)
      M_DoSave(saveSlot);

	else if (savegamestrings[saveSlot][0] && NeedsNameInput == true)
	  {
		  M_SetupNextMenu(&NewDef);
	  }
  }
    ...
    currentMenu->menuitems[itemOn].status)
  {
    ...
    if (currentMenu->menuitems[itemOn].status == 2)
    ...
    else
      {
		...
		if(!svMenu.gotvoice)
        S_StartSound(NULL,sfx_swknob);	//kaiser
      }
  }
      //jff 3/24/98 remember last skill selected
...
// M_Ticker
//
void M_Ticker (void)	//8 cursor hack - kaiser
{
...
void M_WriteDialog(int x,int y,int w,const char* string)
{
		...
		if (string[i] == '\n')
	{
		...
		M_WriteText(cx, cy, cs);
		cy += M_StringHeight(cs);
		start = i + 1;
		wordStart = true;
	}
	else if (wordStart && string[i] > ' ')
	{
		...
		if (M_StringWidth(cs) > w)
		{
			memset(cs, 0, sizeof(cs));
			strncpy(cs, string + start, i - start);
			M_WriteText(cx, cy, cs);
			cy += M_StringHeight(cs) + 4;
			start = i;
	}
		wordStart = false;
	}
		...
		if (!string[i + 1])
		{
			M_WriteText(cx, cy, string + start);
		}
	}
}
