...
void R_DrawVisSprite(vissprite_t *vis, int x1, int x2)
{
  ...
  //because of the limits, I've cheaped this code to support multiple color tables for
  //the players, guards, and peasants...its cheap but it works...-kaiser
  if (!dc_colormap)   // NULL colormap = shadow draw
	...
	else if (vis->mobjflags & MTF_SWAP1)
      {
        colfunc = R_DrawTranslatedColumn;
        dc_translation = translationtables - 256 + (4<<8);
      }
	else if (vis->mobjflags & MTF_SWAP2)
      {
        colfunc = R_DrawTranslatedColumn;
        dc_translation = translationtables - 256 + (5<<8);
      }
	else if (vis->mobjflags & MTF_SWAP3)
      {
        colfunc = R_DrawTranslatedColumn;
        dc_translation = translationtables - 256 + (6<<8);
      }
	else if (vis->mobjflags & MTF_SWAP4)
      {
        colfunc = R_DrawTranslatedColumn;
        dc_translation = translationtables - 256 + (7<<8);
      }
    else
...
// proff 11/06/98: Changed for high-res
  ...
  if(vis->footclip && !vis->psprite)	//kaiser
	{
...
void R_AddSprites(subsector_t* subsec, int lightlevel)
{
  ...
  for (thing = sec->thinglist; thing; thing = thing->snext)
	  if((thing->flags & MF_DONTDRAW)==0)	//kaiser
...
void R_DrawPSprite (pspdef_t *psp)
{
  ...
  // kaiser: make the sprites move based on the player's look direction
  vis->texturemid = ((BASEYCENTER + viewplayer->lookdir / 16)<<FRACBITS)+FRACUNIT/2
...
// proff 11/06/98: Added for high-res
  ...
  {
    vis->colormap = fullcolormap;                    // shadow draw
    vis->mobjflags |= MF_TRANSLUCENT;	//kaiser
  }
  else if (fixedcolormap)
...
#ifndef GL_DOOM
	//fix by Quasar - makes the sprites y position absolute.
	oldcentery = centery;
