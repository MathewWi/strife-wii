...
void WI_updateDeathmatchStats(void)
{
  ...
  {
    if (!(bcnt&3))
      S_StartSound(0, sfx_radio);  // noise while counting
...
void WI_Ticker(void)
{
  ...
  {
    // intermission music
    if ( gamemode == commercial )	//left everything alone..-kaiser
      S_ChangeMusic(mus_end, true);
    else
      S_ChangeMusic(mus_end, true);
  }
  ...
  switch (state)
  {
    case StatCount:
         if (deathmatch)
           ...
           WI_drawStats();	//FIXME - causing crashes?! -kaiser
         break;
