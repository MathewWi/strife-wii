...
int mapcolor_plyr[4] = { 128, 176, 48, 64 }; // colors for player arrows in multiplayer
...
( event_t*  ev )
{
  ...
  else if (ev->type == ev_keydown)
  {
    ...
    else if (ch == key_map_follow)
    {
	  ...
	  doom_printf("%s", (automapmode & am_follow) ? s_AMSTR_FOLLOWON : s_AMSTR_FOLLOWOFF);
    }
    else if (ch == key_map_grid)
    {
	  ...
	  doom_printf("%s",(automapmode & am_grid) ? s_AMSTR_GRIDON : s_AMSTR_GRIDOFF);
    }
  ...
  int  colorrange)
{
  ...
  AM_rotate(&x, &y, ANG90-plr->mo->angle, plr->mo->x, plr->mo->y);
	  ...
	  switch(t->flags)
        {
	  case MF_SOLID:	case MF_NOBLOCKMAP:
            AM_drawLineCharacter
            (
              thintriangle_guy,
      ...
      (
        ...
        t->radius,		//kaiser
        t->angle,
        t->flags & MF_COUNTKILL && MF_MISSILE ? 64 : 224,
        x, y
      );
      t = t->snext;
    }
...
void AM_Drawer (void)
{
  ...
  AM_drawWalls();
  AM_drawPlayers();
  if (ddt_cheating==2 || plr->powers[pw_allthings])
