 ...
 *-----------------------------------------------------------------------------
 *
 * $Id: p_user.c,v 1.4.2.2 2002/07/20 18:08:37 proff_fs Exp $
 *
...
static const char
rcsid[] = "$Id: p_user.c,v 1.4.2.2 2002/07/20 18:08:37 proff_fs Exp $";
...
void P_MovePlayer (player_t* player)
{
  ...
  onground = (mo->z <= mo->floorz || mo->onmobj == true);	//kaiser
  ...
  look = cmd->lookfly&15;
	...
	/*1-26-06	fixed lookmove from 'jerking' if the player tried to look
	all the way down or up		-kaiser*/
	if(nrNum10(player->lookdir) < 190 && cmd->lookmove > 0)
	{
		if(player->lookdir + cmd->lookmove > 190)
			player->lookdir = 190;
		else
		player->lookdir += cmd->lookmove;
	}
  ...
  if (cmd->buttons & BT_JUMP
	  ...
	  && player->mo->momz == 0 && (player->mo->z == player->mo->floorz ||
	  player->mo->onmobj == true))//updated 10-10-05
    {
            player->mo->momz = JUMPGRAVITY/2;
            player->mo->momz = JUMPGRAVITY;
                player->jumpdown |= 1;
				player->jumptics = 27;
				player->mo->onmobj = false;
    }

  if(cmd->arti)
  {
	  if(player->inventory.invTime == 0)
	  {
	  if(cmd->arti&AFLAG_INVLEFT)
	  {
		  P_InvLeft(player);
		  player->inventory.invTime = 4;
		  player->inventory.invDecide = 25;
	  }

	  if(cmd->arti&AFLAG_INVRIGHT)
	  {
		  P_InvRight(player);
		  player->inventory.invTime = 4;
		  player->inventory.invDecide = 25;
	  ...
	  if(cmd->arti&AFLAG_DROP)
	  {
		  P_TossInventory(player, false);
		  player->inventory.invTime = 4;
	  }

	  if(cmd->arti&AFLAG_USEINV)
	  {
		  P_InvUse(player);
		  player->inventory.invTime = 8;
	  }
	  }
  }
}
...
void P_DeathThink (player_t* player)
  ...
  if(gamemap == 29)	//die at the last map and get the dark ending -kaiser
  {
	  finaletype = dark;
	  D_StartEndPic();
  }
...
void P_PlayerThink (player_t* player)
  ...
  if(player->weaponowned[wp_crossbow] && player->ammo[am_poisarrows] > 0)
  {
	  player->weaponowned[wp_crossbow2] = true;
  }

  if(player->weaponowned[wp_grenade] && player->ammo[am_grenades2] > 0)
  {
	  player->weaponowned[wp_grenade2] = true;
  }

  if(player->jumptics)	//kaiser
	{
		player->jumptics--;
	}
  if(player->misctime)
  ...
  if(player->inventory.invTime)
  {
	  player->inventory.invTime--;
  }
  if(player->inventory.invDecide)
  {
	  player->inventory.invDecide--;
  }
  if(player->burntics)
  {
	  player->burntics--;
	  if(player->burntics == 0)
	  {
		player->playerstate = PST_DEAD;
		P_DropWeapon (player);
		return;
  ...
  if(player->lastNpcTic)
  {
	  P_UseLines(player);	//make sure the npc is activated after a slideshow
	  player->lastNpcTic--;
  }
  if(player->misctime2)
  {
	  player->misctime2--;
	  if (!(leveltime & 31) && player->misctime2 > 16*TICRATE)
	  {
  ...
  if (player->powers[pw_targeter] > 0)
  ...
  player->fixedcolormap = player->powers[pw_invulnerability] > 4*32 ||
	...
	if(player->misctime && player->readyweapon == wp_sigil)
	{
