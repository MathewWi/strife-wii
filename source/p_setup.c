 ...
 *-----------------------------------------------------------------------------
 *
 * $Id: p_setup.c,v 1.15.2.2 2002/07/21 11:59:55 cph Exp $
 *
...
static const char
rcsid[] = "$Id: p_setup.c,v 1.15.2.2 2002/07/21 11:59:55 cph Exp $";
...
void LoadRogueConScript(const char *LumpName)
{
	...
	//	Load them.
	LevelSpeeches = (RogueConSpeech_t *)W_CacheLumpNum(LumpNum);
	NumLevelSpeeches = W_LumpLength(LumpNum) / sizeof(RogueConSpeech_t);
}

void LoadGlobalScript(const char *LumpName)	//for the random stuff -kaiser
{
	int LumpNum;
	//	Clear variables.
	GenericSpeeches = NULL;
	NumGenericSpeeches = 0;
	...
	//	Load them.
	GenericSpeeches = (RogueConSpeech_t *)W_CacheLumpNum(LumpNum);
	NumGenericSpeeches = W_LumpLength(LumpNum) / sizeof(RogueConSpeech_t);
}
...
static void P_LoadLineDefs2(int lump)
{
  ...
  if ((ld->sidenum[1] == -1) && (ld->flags & ML_TWOSIDED)) {
    ...
    lprintf(LO_WARN, "P_LoadSegs: linedef %d has two-sided flag set, but no second sidedef\n",numlines-i);
	  ...
	  if (ld->flags & ML_TRANSPARENT)	//new flag defined for transparent textures - kaiser
	  {
		  int lump, j;

		  lump = sides[*ld->sidenum].flags; // translucency from sidedef
            if (!ld->tag)                       // if tag==0,
              ld->tranlump = lump;              // affect this linedef only
            else
              for (j=0;j<numlines;j++)          // if tag!=0,
	  ...
	  if (ld->flags & ML_JUMPOVER)		//kaiser
	  {
		  ld->flags &= ~ML_BLOCKING;
	  }

	  //convert line specials to boom		//kaiser
	  switch(ld->special)
	  {
		case 146:
		//case 209:
...
// proff 11/99: clean the memory from textures etc.
  ...
  sprintf(R_lumpname,"SCRIPT%02d",map);
  LoadGlobalScript("SCRIPT00");
  LoadRogueConScript(R_lumpname);
