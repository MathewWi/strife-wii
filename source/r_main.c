 ...
 *-----------------------------------------------------------------------------
 *
 * $Id: r_main.c,v 1.14.2.1 2002/07/20 18:08:37 proff_fs Exp $
 *
...
static const char rcsid[] = "$Id: r_main.c,v 1.14.2.1 2002/07/20 18:08:37 proff_fs Exp $";
...
void R_SetupFrame (player_t *player)
{
  ...
  tempCentery = viewheight/2+(player->lookdir)*screenblocks/10;
	if(centery != tempCentery)
	{
		...
		centeryfrac = centery<<FRACBITS;
		/*Heretic's 
		code caused plane errors, so I decided to check out 
			R_ExecuteSetViewSize and based the code on that. 
			So far it works very well - kaiser*/
		for(i = 0; i < viewheight; i++)
		{
			yslope[i] = FixedDiv (projectiony,
				D_abs(((i-centery)<<FRACBITS)+FRACUNIT/2));
		}
	}
