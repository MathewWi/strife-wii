...
{
	{ "F_WATR01", FLOOR_WATER },
	{ "F_PWATR1", FLOOR_SLUDGE },
	{ "END", -1 }
};
...
// P_InitPicAnims
//
//strife support added - kaiser

animdef_t               harddefs[] =
{
    // Strife flat animations.
    {false,     "F_CONVY2",      "F_CONVY1",      8},
    {false,     "F_FAN2",        "F_FAN1",      8},
    {false,     "F_HWATR2",      "F_HWATR1",      8},
    {false,     "F_PWATR3",      "F_PWATR1",        8},
    {false,     "F_RDALN4",       "F_RDALN1",       8},
    {false,     "F_SCANR4",      "F_SCANR1",      8},
    {false,     "F_SCANR8",      "F_SCANR5",      8},
    {false,     "F_VWATR3",      "P_VWATR1",      8},
    {false,     "F_WATR03",      "F_WATR01",      8},

    // Strife animated textures
    {true,      "SCAN04",      "SCAN01",      8},
	{true,      "SCAN08",      "SCAN05",      8},
	{true,		"BUBL04",	   "BUBL01",	  8},
	{true,		"COMP04",	   "COMP01",	  6},
	{true,		"COMP08",	   "COMP05",	  6},
	{true,		"COMP12",	   "COMP09",	  6},
	{true,		"COMP16",	   "COMP13",	  6},
	{true,		"COMP20",	   "COMP17",	  6},
	{true,		"COMP24",	   "COMP21",	  6},
	{true,		"COMP28",	   "COMP25",	  6},
	{true,		"COMP32",	   "COMP29",	  6},
	{true,		"COMP37",	   "COMP33",	  6},
	{true,		"COMP41",	   "COMP38",	  6},
	{true,		"COMP49",	   "COMP42",	  6},
	{true,		"FORCE04",	   "FORCE01",	  8},
	{true,		"FORCE08",	   "FORCE05",	  8},
	{true,		"SWTRMG03",	   "SWTRMG01",	  8},
	{true,		"BRNSCN04",		"BRNSCN01",		8},
	{true,		"BRKGRY16",		"BRKGRY13",		8},
	{true,		"WALPMP02",		"WALPMP01",		8},
	{true,		"FAN02",		"FAN01",		8},
	{true,		"WALTEK17",		"WALTEK16",		8},
	{true,		"CONCRT25",		"CONCRT22",		8},
	{true,		"CONCRT12",		"CONCRT09",		8},

    {-1}
};
#define MAXLINEANIMS            64//kaiser
extern  short	numlinespecials;	//kaiser
extern  line_t*	linespeciallist[MAXLINEANIMS];//kaiser
//okay, why I removed the anidefs/switches lump? like everyone who maps will use them
//anyways...so why bother. - kaiser
...
// P_BreakingGlass		//kaiser
void P_BreakingGlass(line_t* li)
{
	fixed_t x;
	fixed_t y;
	...
	x = li->v1->x + li->dx/2;
	y = li->v1->y + li->dy/2;
	P_SpawnMobj (x, y, ONFLOORZ, MT_GLASS);
	//9-11-05 here's a hack that I missed -kaiser
	if(sides[li->sidenum[0]].midtexture == R_TextureNumForName("BRNSCN01"))
	{
		players[0].accuracy += 10;
		players[0].stamina += 10;
		players[0].maxhealth += 10;
		players[0].quest |= 1 << 29;
	}
	//11-9-05 and another one...
	if(li->special == 180)
	{
		EV_DoFloor(li, raise512Change);
	}
	P_ChangeSwitchTexture(li, 0);			//panel/window is broken..
	li->flags &= ~ML_BLOCKING;				//let the player go though a window
	li->flags &= ~ML_BLOCKALL;
}
...
//    case 156:
    ...
    case 195:               // Thing teleporters	//kaiser
    case 97:
	case 231:
    case 39:
    case 126:
...
//======================================================
//Kaiser's custom log function
//The purpose of this is to allow custom text messages, since the real editing spec has
//been lost...
//======================================================
char LogMessage[MAX_TEXT_SIZE];
char *GetLog(int sequence)
{
	char *msg;
	int logLump;
	int logSize;
	static char *MsgLogNames[] =
	{
		"s_log0", "s_log1", "s_log2", "s_log3", "s_log4", "s_log5", "s_log6", "s_log7",
		"s_log8", "s_log9", "s_log10", "s_log11", "s_log12", "s_log13", "s_log14",
		"s_log15","s_log16", "s_log17", "s_log18", "s_log19", "s_log20"
	};
	msg = MsgLogNames[sequence];
	logLump = W_GetNumForName(msg);
	logSize = W_LumpLength(logLump);
	if(logSize >= MAX_TEXT_SIZE)
				{
					I_Error("Log Message is too long (%s)", msg);
				}
	W_ReadLump(logLump, LogMessage);
	LogMessage[logSize] = 0; // Append terminator
	return LogMessage;
}
...
void P_CrossSpecialLine(line_t *line, int side, mobj_t *thing)
{
  ...
  //char combuf[9]; *fix
  int questforline;
  questforline = sides[line->sidenum[0]].rowoffset >> FRACBITS;	//kaiser
  ...
  if (!thing->player)
  {
    ...
    switch(line->special)
    {
      ...
      case 185:     //silent thing teleporters	//kaiser
  ...
  sprintf(namebuf, "VOC%i", line->tag);
 // sprintf(combuf, "LOG%i", thing->player->objectives); *fix
  switch(line->special)
  {
	  case 150:
		  //Alert Trigger		-kaiser
		  if(thing->player)
            P_NoiseAlert(thing->player->mo,thing->player->mo);
            break;
	  case 198:
		  //Alarm if has no Officer Uniform
		  if(thing->player && !thing->player->inventory.mobjItem[MT_QUEST_GUARD_UNIFORM])
		  {
			  P_NoiseAlert(thing->player->mo,thing->player->mo);
		  }
		  break;
	  case 208:
		   //Alarm if has flamethrower
		  if(thing->player && thing->player->weaponowned[wp_flamethrower])
		  {
			  P_NoiseAlert(thing->player->mo,thing->player->mo);
			  line->special = 0;
		  }
	...
	case 206:
		  //Alert Trigger		-kaiser
		  if(thing->player && thing->player->inventory.mobjItem[MT_INV_CHALICE])
            P_NoiseAlert(thing->player->mo,thing->player->mo);
            break;

	case 212:	//Lower Floor if have flamethrower.
		  if(thing->player && thing->player->weaponowned[wp_flamethrower])
		  {
			  line->special = 0;
	  ...
	  case 250:	//kaiser
		  if(thing->player)
		  {
			  ...
			  //P_SetMessage(thing->player, GetLog(line->tag));
			  S_StartSound(thing->player,sfx_radio);
			  line->special = 0;
		  }
		  break;
	  case 228:
			  S_StartVoice(thing, "VOC130");
			  line->special = 0;
		  break;

	  case 204:
			S_StartMusic(line->tag);
			line->special = 0;
		  break;
	  ...
	  case 202:
	  case 210:
		  if(thing->player && thing->player->communicator == true)
		  {
			  doom_printf("Incoming Message...");
			  //P_SetMessage(thing->player,"Incoming Message...");
			  thing->player->objectives = line->tag;
			  line->special = 0;
			  if(W_CheckNumForName(namebuf) == -1)
				return;
			  else
			  S_StartVoice(thing,namebuf);
		  }
		  break;

	  case 215:	//special quest line			-kaiser
		  if(thing->player && thing->player->communicator == true
			  && (thing->player->quest & (1 << (line->tag - (line->tag/100 * 100)))))
		  {
			  line->special = 0;
			  doom_printf("Incoming Message From Blackbird");
			  thing->player->objectives = line->tag/100;
			  sprintf(namebuf, "VOC%i", line->tag/100);
			  if(W_CheckNumForName(namebuf) == -1)
				return;
			  else
			  S_StartVoice(thing,namebuf);
		  ...
		  break;
	  case 193:	//Quest Line. Lowers Floor depending on Quest Type = Y offset
		  if(questforline == 0 && !netgame)
		  {
			  return;
		  }
		  if(thing->player && thing->player->quest & (1 << questforline ))
		  {
			  line->special = 0;
			  EV_DoFloor(line, lowerFloorToLowest);
		  }
		  break;
	  case 230:	//Quest Line. Open Door depending on Quest Type = Y offset
		  if(questforline == 0 && !netgame)
		  {
			  return;
		  }
		  ...
		  break;
	  case 216:	//Quest Line. Close Door depending on Quest Type = Y offset
		  if(questforline == 0 && !netgame)
		  {
			  return;
		  }
		  ...
		  break;
	  case 227:	//Quest Line. Close Door depending on Quest Type = Y offset
		  if(questforline == 0 && !netgame)
		  {
			  return;
		  }
	  ...
	  case 187:	//Quest Line. Turns off force field on Quest Type = Y offset
		  if(questforline == 0 && !netgame)
		  {
			  return;
		  }
		  ...
		  {
			  line->special = 0;
			  EV_DestoryLineShield(line,0);
		  }
		  break;
	  case 188:	//Quest Line.
		  if(thing->player && thing->player->quest & (1 << TOKEN_QUEST16) || netgame)
		  {
			  line->special = 0;
			  EV_DoDoor(line,open);
		  }
		  break;
	  case 200:	//Quest Line.
		  if(thing->player && thing->player->weaponowned[wp_sigil] || netgame)
		  {
			  line->special = 0;
			  EV_DoDoor(line,open);
		  }
  ...
  switch (line->special)
  {
	...
	case 183:
	  if (EV_DoFloorAndCeiling(line, lowerFloorToNearest, raiseToNearest))
		  line->special = 0;
	  break;
	...
	case 197:
      // Blazing Door Close requires all sigils
    ...
    default:
      if (!demo_compatibility)
        switch (line->special)
        {
          ...
          case 145:	//Teleport To New Map/Spot
            if (!(thing->player && thing->player->health <= 0))
			{
				G_ExitLevel (line->tag/100,line->tag - (line->tag/100 * 100));
			}
            break;

          case 186:	//Teleport To New Map/Spot - 1 side
            if (!(thing->player && thing->player->health <= 0) && side != 1)
			{
				G_ExitLevel (line->tag/100,line->tag - (line->tag/100 * 100));
			}
            break;
...
void P_MoveThing(mobj_t* thing,angle_t angle,fixed_t move)	//P_Thrust for everybody!
{
  angle >>= ANGLETOFINESHIFT;
  thing->momx += FixedMul(move,finecosine[angle]);
  thing->momy += FixedMul(move,finesine[angle]);
}

void P_MoveObjectInSector (mobj_t* thing)	//kaiser
{
	int	scrolldir;	//kaiser
	int	east,eastn,north,northw,west,wests,south,southe,i;
	int	speed;
	sector_t*   sector;
	sector = thing->subsector->sector;

	speed = 0;
		  for(east=100; east < 110; east++)
		  {
			  if(sector->tag == east)
			  {
			  scrolldir = 0;
		  ...
		  for(northw=130; northw < 140; northw++)
		  {
			  if(sector->tag == northw)
			  {
			  scrolldir = ANG90+ANG45;
			  speed = (northw-130);
			  }
		  }
		  for(west=140; west < 150; west++)
		  {
			  if(sector->tag == west)
			  {
			  scrolldir = ANG180;
		  ...
		  for(wests=150; wests < 160; wests++)
		  {
			  if(sector->tag == wests)
			  {
			  scrolldir = ANG180+ANG45;
			  speed = (wests-150);
			  }
		  }
		  for(south=160; south < 170; south++)
		  {
			  if(sector->tag == south)
			  {
			  scrolldir = ANG270;
		  ...
		  for(southe=170; southe < 180; southe++)
		  {
			  if(sector->tag == southe)
			  {
			  scrolldir = ANG270+ANG45;
			  speed = (southe-170);
			  }
		  }
		  for(i=180; i < 190; i++)
		  ...
		  //doom_printf("%d",speed);
		  P_MoveThing(thing, scrolldir, 2048*(speed+3));
}
...
void P_PlayerInSpecialSector (player_t* player)
{
  ...
  if (sector->special<32) // regular sector specials
  {
    switch (sector->special)
	  ...
	  case 15:
		// Instant Death to any who touches this sector!		-kaiser
		//player->cheats &= ~CF_GODMODE;
		P_DamageMobj (player->mo, NULL, NULL, 1000000);
		break;

	  case 18:		//scrollers using linedef tags to push in direction -kaiser
		  P_MoveObjectInSector(player->mo);
		  break;
...
void P_UpdateSpecials (void)
{
  ...
  //	ANIMATE LINE SPECIALS		//the special up/down texture -kaiser
    for (i = 0; i < numlinespecials; i++)
    {
		line = linespeciallist[i];
		switch(line->special)
		{
		/*case 2561: // Scroll_Texture_Up
				sides[line->sidenum[0]].rowoffset += FRACUNIT;
				break;*/
		case 143: // Scroll_Texture_Down
				sides[line->sidenum[0]].rowoffset -= FRACUNIT;
				break;
		}
    }
...
//
short		numlinespecials;	//kaiser
line_t*		linespeciallist[MAXLINEANIMS];	//kaiser
  ...
  for (i=0;i<numlines;i++,l++)
    {
      ...
      switch (special)
        ...
        case 870:   // scroll effect ceiling
          for (s=-1; (s = P_FindSectorFromLineTag(l,s)) >= 0;)
            Add_Scroller(sc_ceiling, -dx, dy, control, s, accel);
          break;

        case 871:   // scroll effect floor
        ...
        case 149:                  // jff 1/30/98 2-way scroll
          Add_Scroller(sc_side, -FRACUNIT, 0, -1, lines[i].sidenum[0], accel);
          break;
        }
    }
