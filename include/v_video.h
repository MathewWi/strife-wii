...
typedef enum
{
  ...
  CR_VIOLET,    //7
  CR_YELLOW,  //8
  CR_LIMIT    //9 //jff 2/27/98 added for range check
} crange_idx_e;
//jff 1/16/98 end palette color range additions

#define CR_DEFAULT CR_GOLD   /* default value for out of range colors */
...
enum patch_translation_e {
  ...
  VPT_TRANSLUCENT = 5,	//stretches and draws transparent patch		-kaiser
};
...
void V_DrawFuzzPatch (int x, int y, int scrn, const patch_t *patch, enum patch_translation_e flags);	//kaiser
void V_DrawNumFuzzPatch(int x, int y, int scrn, int lump, enum patch_translation_e flags);
#define V_DrawNameFuzzPatch(x,y,s,n,f) V_DrawNumFuzzPatch(x,y,s,W_GetNumForName(n),f)
